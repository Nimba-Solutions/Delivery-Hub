...
export default class DeliveryHubBoard extends NavigationMixin(LightningElement) {
    ...
    _autoEnhanceTimer = null;
    _autoEnhanceTriggered = false;
    ...
    get hasMetrics()                { return this.showBoardMetrics && this.boardMetrics != null; }
    ...
    handleCreateCancel() {
        ...
        this._autoEnhanceTriggered = false;
        clearTimeout(this._autoEnhanceTimer);
    }
    ...
    handleFieldChange(event) {
        ...
        // Auto-enhance: debounce AI call when title has enough content
        if (this.AutoGenerateDescriptions && this.AiEnhancementEnabled
            && fieldName === FIELDS.BRIEF_DESC && !this._autoEnhanceTriggered) {
            clearTimeout(this._autoEnhanceTimer);
            const title = (fieldValue || '').trim();
            if (title.length >= 5) {
                this._autoEnhanceTimer = setTimeout(() => {
                    this._autoEnhanceTriggered = true;
