...
const INFO_REGISTRY = {
    ...
    deliveryPacingForecast: {
        dataSource:
            'Calls DeliveryHoursAnalyticsController.getPortfolioPacing(granularity, periodsBack, periodsForward). ACTUALS: per-period logged bars and the Logged headline count ALL WorkLog__c hours by WorkDateDate__c — no active-portfolio filter — so completed/archived work is never dropped (the bars equal a raw GROUP BY CALENDAR_x(WorkDateDate__c) total). FORECAST is schedule-based: each in-flight WorkItem__c (active, non-terminal, non-archived, non-template) has remaining = max(0, EstimatedHoursNumber__c − logged-so-far), and that remaining is spread evenly across the forward periods its EstimatedStartDevDate__c → EstimatedEndDevDate__c span covers — the SAME two date fields the Gantt drag-writes, so the two views can\'t drift. Remaining that can\'t be placed (no usable dates, or end date in the past) is summed into unscheduledRemainingHours and surfaced as a note rather than dropped. Projected final = total logged + Σ remaining. An amortized target line (estimate across earliest-start → latest-end) is kept as a faint secondary reference. The blended $ rate is surfaced only when exactly one active NetworkEntity__c carries a DefaultHourlyRateCurrency__c (else hours-only).',
        description:
            'Account/org-level hours & forecast across all active projects. Hours-first: logged hours per period (bars) and a schedule-based forecast (dashed bars) that spreads each in-flight item\'s remaining estimate across the months its scheduled start→end covers. Un-dated remaining work that can\'t be placed shows as a note prompting you to set dates on the Gantt. Week/Month/Quarter bucket selector and a forward horizon (Next 3 / 6 / 12 or rest of year). Dollars appear secondary when a single blended rate is configured.',
        friendlyName: 'Portfolio Hours & Forecast',
        keyFields:
