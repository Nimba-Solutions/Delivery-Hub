...
export default class DeliveryDhtmlxGantt extends NavigationMixin(LightningElement) {
    ...
    _initGantt() {
        ...
        ganttInstance.config.columns = [
            {
                ...
                template: function(task) {
                    if (task.type === ganttInstance.config.types.project) {
                        return '<span class="dhtmlx-gantt-project-label">' + task.text + '</span>';
                    }
                    return '<span class="dhtmlx-gantt-task-name">' + task.text + '</span>';
                }
            },
            {
                name: 'stage',
                ...
                template: function(task) {
                    ...
                    return '<span class="dhtmlx-gantt-stage-pill">' + (task.stage || '') + '</span>';
                }
            }
        ];
        ...
        ganttInstance.templates.task_class = function(start, end, task) {
            if (task.isCompleted) { return 'dhtmlx-gantt-task-completed'; }
            return '';
        };
        ...
        ganttInstance.templates.task_text = function(start, end, task) {
            ...
            const hours = task.loggedHours != null && task.estimatedHours != null
                ...
                : '';
            return '<span class="dhtmlx-gantt-bar-text">' + hours + '</span>';
        };
