...
// Maps each AI toggle's state-prop name to its DeliveryHubSettings__c field API name,
// so the consequence-preview copy can be looked up from the shared Apex source.
const FIELD_API_BY_PROP = {
    aiSuggestionsEnabled: 'EnableAISuggestionsDateTime__c',
    aiEstimationEnabled: 'EnableAIEstimationDateTime__c',
    autoGenerateDescriptions: 'EnableAutoGenerateDescriptionsDateTime__c'
};
...
export default class DeliveryAiSettingsCard extends LightningElement {
    ...
    async handleSettingChange(event) {
        ...
        // --- CONSEQUENCE PREVIEW ---
        // Enabling an AI toggle sends content to OpenAI and incurs usage cost — show
        // a plain-English confirm BEFORE committing. Cancel reverts (stays OFF).
        if (checked) {
            const confirmed = await confirmConsequence(FIELD_API_BY_PROP[name]);
            if (!confirmed) {
                event.target.checked = false;
                return;
            }
